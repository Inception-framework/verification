...
def execute_on_device_and_dump(id,changed_regs):
    ...
    # adapt to klee: always dump SP
    sp = list(device.regs.keys()).index('SP')
    if not sp in changed_regs:
      changed_regs.append(sp);   
    ...
    sppc_file.close()
    SP = int(stack_start + (stack_end - stack_start) / 2) - 4
